  ...
  static blockModifier = 4;
	...
	constructor(players, npcs, ID, techList) {
      ...
      for(let i = 0; i < players.length; i++) {
        ...
        for(let j = 0; j < npcs.length; j++) {
          this.pCombatants[i].threatlist.push([j,npcs[j].battleCurrAtt.con*4]);
        }
      }
      ...
      for(let i = 0; i < npcs.length; i++) {
        ...
        for(let j = 0; j < players.length; j++) {
          this.NPCombatants[i].threatlist.push([j,players[j].battleCurrAtt.con*4]);
        }
      }
    ...
    teamDead(set) {
      for(let i = 0; i < set.length; i++) {
        //if(this.deathChance > 0 !== 0 && (set[i].race.raceName === "Saiyan" || set[i].race.raceName === "Half-Saiyan")) set[i].zenkaiTriggered = 1;
...
/********************************************************************************/
    ...
    nAI(personality, npc, enemies, party) {
      ...
      else if(npc.battleCurrAtt.energy < npc.battleMaxAtt.energy * 0.2) {
        choices[3][1] -= 2;
      }
      else {
      ...
      if(npc.techniques.length === 0) {
        ...
        for(let i = 0; i < enemies.length; i++) {
          ...
          let current = npc.threatlist.map(function(e) { 
            return e[0]; 
          }).indexOf(i);
          let threat = npc.threatlist[current][1]/npc.threatlist[0][1];//herez
        ...
        for(let i = 0; i < party.length; i++) {
          ...
          for(let j = 0; j < heals.length; j++) {
            ...
            let healmod = 1;
            if(parseInt(heals[j].health) == 1) healmod *= 1.25*((party[i].battleMaxAtt.health-party[i].battleCurrAtt.health)/party[i].battleMaxAtt.health);
            if(parseInt(heals[j].energy) == 1) healmod *= 0.25*((party[i].battleMaxAtt.energy-party[i].battleCurrAtt.energy)/party[i].battleMaxAtt.energy);
            if(healmod > highestRecover) highestRecover = healmod;
            heal *= healmod;
            if(heals[j].energyCost > 0) heal *= (npc.battleCurrAtt.energy-scaler*heals[j].energyCost)/npc.battleCurrAtt.energy;
            if(heals[j].healthCost > 0) heal *= (npc.battleCurrAtt.energy-scaler*heals[j].healthCost)/npc.battleCurrAtt.health;
      ...
      for(let i = 0; i < this.actions.length; i++) {
        ...
        if(this.pCombatants[this.actions[i][2]].battleCurrAtt.health > 0) {
          if(this.actions[i][0] === -1) {
            ...
            this.pCombatants[this.actions[i][2]].statRegen();
            for(let j = 0; j < this.NPCombatants.length; j++) {
              let index = this.NPCombatants[j].threatlist.map(function(e) { return e[0]; }).indexOf(this.actions[i][2]);
              this.NPCombatants[j].threatlist[index][1] *= 1.25;
              this.NPCombatants[j].threatlist.sort(function(a,b) {return b[1] - a[1] });
            }
          }
            ...
            for(let j = 0; j < this.NPCombatants.length; j++) {
              let index = this.NPCombatants[j].threatlist.map(function(e) { return e[0]; }).indexOf(this.actions[i][2]);
              this.NPCombatants[j].threatlist[index][1] *= 1.5;
              this.NPCombatants[j].threatlist.sort(function(a,b) {return b[1] - a[1] });
            }
          }
            ...
            if(this.actions[i][3] >= this.NPCombatants.length || this.actions[i][0] === 0) { }
              ...
              let nindex = this.NPCombatants[this.actions[i][3]].threatlist.map(function(e) { return e[0]; }).indexOf(this.actions[i][2]);
              this.NPCombatants[this.actions[i][3]].threatlist[nindex][1] += Math.round(this.pCombatants[this.actions[i][2]].battleCurrAtt.stotal/18);
              this.NPCombatants[this.actions[i][3]].threatlist.sort(function(a,b) {return b[1] - a[1] });
              ...
              if(this.NPCombatants[this.actions[i][3]].guarded === -1) this.NPCombatants[this.actions[i][3]].takeDamage(this.actions[i][0]);
                ...
                if(this.NPCombatants[index].battleCurrAtt.health <= 0) {
                  ...
                  let sindex = this.NPCombatants[index].threatlist.map(function(e) { return e[0]; }).indexOf(this.actions[i][2]);
                  this.NPCombatants[index].threatlist[sindex][1] += Math.round(this.pCombatants[this.actions[i][2]].battleCurrAtt.stotal/6);
                  this.NPCombatants[index].threatlist.sort(function(a,b) {return b[1] - a[1] });
                }
                else {
                  ...
                  let sindex = this.NPCombatants[index].threatlist.map(function(e) { return e[0]; }).indexOf(this.actions[i][2]);
                  this.NPCombatants[index].threatlist[sindex][1] += Math.round(this.pCombatants[this.actions[i][2]].battleCurrAtt.stotal/12);
                  this.NPCombatants[index].threatlist.sort(function(a,b) {return b[1] - a[1] });
              ...
              if(this.NPCombatants[this.actions[i][3]].battleCurrAtt.health <= 0) {
                ...
                for(let j = 0; j < this.NPCombatants.length; j++) {
                  let index = this.NPCombatants[j].threatlist.map(function(e) { return e[0]; }).indexOf(this.actions[i][2]);
                  this.NPCombatants[j].threatlist[index][1] *= 1.75;
                  this.NPCombatants[j].threatlist.sort(function(a,b) {return b[1] - a[1] });
                }
              }
            }
      ...
      for(let i = 0; i < this.NPCactions.length; i++) {
        ...
        if(this.NPCombatants[this.NPCactions[i][2]].battleCurrAtt.health > 0) {
          if(this.NPCactions[i][0] === -1) {
            ...
            this.NPCombatants[this.NPCactions[i][2]].statRegen();
            for(let j = 0; j < this.pCombatants.length; j++) {
              let index = this.pCombatants[j].threatlist.map(function(e) { return e[0]; }).indexOf(this.NPCactions[i][2]);
              this.pCombatants[j].threatlist[index][1] *= 1.25;
              this.pCombatants[j].threatlist.sort(function(a,b) {return b[1] - a[1] });
            }
          }
            ...
            for(let j = 0; j < this.pCombatants.length; j++) {
              let index = this.pCombatants[j].threatlist.map(function(e) { return e[0]; }).indexOf(this.NPCactions[i][2]);
              this.pCombatants[j].threatlist[index][1] *= 1.5;
              this.pCombatants[j].threatlist.sort(function(a,b) {return b[1] - a[1] });
            }
          }
            ...
            this.NPCombatants[this.NPCactions[i][2]].statRegen();
            let nindex = this.pCombatants[this.NPCactions[i][3]].threatlist.map(function(e) { return e[0]; }).indexOf(this.NPCactions[i][2]);
            this.pCombatants[this.NPCactions[i][3]].threatlist[nindex][1] += Math.round(this.NPCombatants[this.NPCactions[i][2]].battleCurrAtt.stotal/18);
            this.pCombatants[this.NPCactions[i][3]].threatlist.sort(function(a,b) {return b[1] - a[1] });
            ...
            if(this.NPCactions[i][3] >= this.pCombatants.length || this.NPCactions[i][0] === 0) { }
              ...
              if(this.pCombatants[this.NPCactions[i][3]].guarded === -1) this.pCombatants[this.NPCactions[i][3]].takeDamage(this.NPCactions[i][0]);
                ...
                if(this.pCombatants[index].battleCurrAtt.health <= 0) {
                  ...
                  let sindex = this.pCombatants[index].threatlist.map(function(e) { return e[0]; }).indexOf(this.NPCactions[i][2]);
                  this.pCombatants[index].threatlist[sindex][1] += Math.round(this.NPCombatants[this.NPCactions[i][2]].battleCurrAtt.stotal/6);
                  this.pCombatants[index].threatlist.sort(function(a,b) {return b[1] - a[1] });
                }
                else {
                  ...
                  let sindex = this.pCombatants[index].threatlist.map(function(e) { return e[0]; }).indexOf(this.NPCactions[i][2]);
                  this.pCombatants[index].threatlist[sindex][1] += Math.round(this.NPCombatants[this.NPCactions[i][2]].battleCurrAtt.stotal/12);
                  this.pCombatants[index].threatlist.sort(function(a,b) {return b[1] - a[1] });
              ...
              if(this.pCombatants[this.NPCactions[i][3]].battleCurrAtt.health <= 0) {
                ...
                for(let j = 0; j < this.pCombatants.length; j++) {
                  let index = this.pCombatants[j].threatlist.map(function(e) { return e[0]; }).indexOf(this.NPCactions[i][2]);
                  this.pCombatants[j].threatlist[index][1] *= 1.75;
                  this.pCombatants[j].threatlist.sort(function(a,b) {return b[1] - a[1] });
                }
              }
            } 
